 ...
 */
export const MozillaObservatoryModuleEvents: any = {
...
export class MozillaObservatoryModule extends AbstractDomainModule {
   ...
   * @private
   */
  private context: WebAuditContextClass | undefined;
   ...
   * {@inheritdoc}
   */
  async analyse(urlWrapper: UrlWrapper) {
    try {
      ...
      const summary = {
        ...
        grade: result.grade,
        score: result.score,
        tests_failed: result.tests_failed, // eslint-disable-line @typescript-eslint/naming-convention
   ...
   */
  async getBaseResult(domain: string): Promise<any> {
   ...
   * @returns {Promise<void>}
   */
  async getTestResults(scanId: string): Promise<any> {
    if (scanId) {
